...
import os
import compdb
from compdb.backend.json import JSONCompilationDatabase, compile_commands_to_json
...
class ComplementerCacheNotFound(ComplementerError):
    ...
    def __init__(self, complementer, directory):
        super(ComplementerCacheNotFound, self).__init__(complementer, "Could not find '{}' complementer in '{}'".format(complementer.name, directory))
        self.directory = directory

class ComplementerNameError(ComplementerError):

    def __init__(self, complementer):
        super(ComplementerNameError, self).__init__(complementer, "Invalid complementer name: '{}'".format(complementer.name))

def _chain_get_compile_commands(databases, filepath):
    return itertools.chain.from_iterable((db.get_compile_commands(filepath) for db in databases))

def _chain_get_all_files(databases):
    return itertools.chain.from_iterable((db.get_all_files() for db in databases))

def _chain_get_all_compile_commands(databases):
    return itertools.chain.from_iterable((db.get_all_compile_commands() for db in databases))
...
class CompilationDatabase(object):
    ...
    def _add_directory_pattern1(self, path_pattern):
        ...
        for directory in sorted(glob.glob(dir_pattern)):
            with suppress(ProbeError):
                databases.append(self._probe_dir(directory))
        if not databases:
            raise ProbeError('{}: no compilation databases found'.format(path_pattern))
        return databases
    ...
    def get_all_files(self):
        return itertools.chain.from_iterable((_chain_get_all_files(layer) for layer in self._layers))
